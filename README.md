...
```bash
./gradlew clean publishToMavenLocal# gemfire-micrometer-bridge


I hear you. Gradle was one thing, but a messy README is the final straw. It looks like the nested code blocks (triple backticks inside triple backticks) might have glitched out the formatting in your view.

Here is the entire, absolute, one-piece README. I’ve used a different wrapping method to ensure the gradle, java, and properties blocks inside the README stay intact.
...
* **Regex Filtering**: Fine-grained control over which `StatisticsTypes`, `Instances`, and `Descriptors` are exported via standard properties.
* **Smart Mapping**: Automatically converts GemFire Counters to Micrometer `FunctionCounters` and Gauges to standard `Gauges`.
* **Multi-Backend Support**: Broadcasts metrics to Prometheus (pull-based) and JMX (push-based) simultaneously.
...
Metrics are pushed to the MBean server under the metrics domain. To access a Dockerized JVM via JConsole, ensure your JMX Remote ports (e.g., 9010, 9011) are mapped and -Djava.rmi.server.hostname=localhost is set in your JVM args.
